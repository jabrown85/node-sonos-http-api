...
function playClipOnAll(player, values) {
  ...
  let clipUrl = `http://${player.system.localEndpoint}:${port}/clips/${clipFileName}`;
  if (/^http/i.test(clipFileName)) {
    clipUrl = decodeURIComponent(clipFileName);
  }

  return allPlayerAnnouncement(player.system, clipUrl, announceVolume);
}
